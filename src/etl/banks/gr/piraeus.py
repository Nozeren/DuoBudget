...
class Piraeus:
    ...
    def extract(self) -> None:
        files_path = [file for file in os.listdir(
            self.upload_folder) if file.endswith(".xlsx")]
    ...
    def clean(self):
        ...
        dataframe.drop(
            dataframe.loc[dataframe["Category"] == "Update Date:"].index, inplace=True)
        ...
        dataframe["Transaction Narrative"] = dataframe["Transaction Narrative"].str.replace(
            "(CARD PURCHASE)", "")
        # Get needed Columns
        dataframe = dataframe[["Posted date",
                               "Transaction Narrative", "Category", "Amount"]]
        ...
        for subcategory in subcategories_list:
            ...
            if result.status_code == 200 and result.json() is not None:
                dataframe.loc[dataframe["Category"] == subcategory, "Category"] = result.json()[
                    'id']
            else:
                dataframe.loc[dataframe["Category"]
