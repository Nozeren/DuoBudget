...
class Santader:
    ...
    def extract(self):
        files_path = [file for file in os.listdir(self.upload_folder) if file.endswith(".xls")]
        # In case it has files
        if files_path:
            self._dataframes = []
            for file in files_path:
                dataframe = pd.read_excel(self.upload_folder + "/" + file, index_col=None, header=6)
    ...
    def clean(self):
        ...
        dataframe["Data Operação"] = pd.to_datetime(dataframe["Data Operação"], format="%d-%m-%Y")
        dataframe["Data Operação"] = dataframe["Data Operação"].dt.strftime('%d/%m/%Y')
        dataframe["Data Operação"] = dataframe["Data Operação"].astype(str)
        dataframe = dataframe[["Data Operação", "Descrição", "Montante( EUR )"]]
