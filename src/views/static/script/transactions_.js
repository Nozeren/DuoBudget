...
let lastMonthYear, firstMonthYear;
let icons = {
...
async function updateTableRows() {
  ...
  for (row of table.getElementsByTagName("td")) {
  ...
  await getTransactions().then(function (data) {
    for (transaction of data) {
      ...
      if (transactionDate <= lastDayOfMonth && transactionDate >= firstDayOfMonth) {
  ...
  // Add Event to cells
  if (lastDayOfMonth >= lastMonthYear.toISOString().split("T")[0]) {
    dateRightButton.hidden = true;
  } else {
    dateRightButton.hidden = false;
  }
  if (firstDayOfMonth <= firstMonthYear.toISOString().split("T")[0]) {
...
window.addEventListener("load", async () => {
  await getTransactions().then(function (data) {
    lastMonthYear = new Date(data[0]["posted_date"]);
    firstMonthYear = new Date(data[0]["posted_date"]);
    for (transaction of data) {
      let currentDate = new Date(transaction["posted_date"]);
      if (lastMonthYear < currentDate) {
        lastMonthYear = currentDate;
      }
      if (firstMonthYear > currentDate) {
        firstMonthYear = currentDate;
      }
    }
  });
  //Add max and min to date picker
  datePicker.setAttribute("value", lastMonthYear.toISOString().slice(0, 7));
  datePicker.setAttribute("max", lastMonthYear.toISOString().slice(0, 7));
  datePicker.setAttribute("min", firstMonthYear.toISOString().slice(0, 7));
  await updateTableRows();
  datePicker.addEventListener("change", () => updateTableRows());
